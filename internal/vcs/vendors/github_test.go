...
func TestGitHubClient_CreateRepo(t *testing.T) {
	tests := []struct {
		name           string
...
func TestGitHubClient_AddWebhook(t *testing.T) {
	tests := []struct {
		name           string
		owner          string
		repo           string
		...
		{
			name:  "webhook creation failure - unauthorized",
			owner: "testuser",
			repo:  "test-repo",
			opts: WebhookOptions{
				URL: "https://example.com/webhook",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				expectedPath := "/repos/" + tt.owner + "/" + tt.repo + "/hooks"
...
func TestGitHubClient_GetActionsProgress(t *testing.T) {
	tests := []struct {
		name           string
		owner          string
		repo           string
				...
				"workflow_runs": []map[string]interface{}{
					{
						...
						"html_url":    "https://github.com/testuser/test-repo/actions/runs/1001",
						"workflow_id": 5,
					},
					{
						"id":          1002,
						...
						"html_url":    "https://github.com/testuser/test-repo/actions/runs/1002",
						"workflow_id": 6,
					},
				},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				expectedPath := "/repos/" + tt.owner + "/" + tt.repo + "/actions/runs"
				if r.URL.Path != expectedPath {
					t.Errorf("Expected path %s, got %s", expectedPath, r.URL.Path)
				}
